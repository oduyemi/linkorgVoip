...
export const Grandstream: React.FC = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      if (selectedBrands.includes("grandstream") || selectedBrands.length === 0) {
        const response = await axios.get(brandUrls["grandstream"]);
  ...
  const handleBrandChange = (brand: string) => {
    if (brand === "grandstream") {
      setSelectedBrands(["grandstream"]);
    } else {
      setSelectedBrands((prevBrands) => {
        const updatedBrands = prevBrands.includes(brand)
          ? prevBrands.filter((b) => b !== brand)
          : [...prevBrands, brand].filter((b) => b !== "grandstream");
        return updatedBrands.length === 0 ? ["grandstream"] : updatedBrands;
      });
    }
  };
      ...
      <Flex flexWrap="wrap" gap={5}>
        ...
        <Box flex="1">
          {loading ? (
            ...
            <GrandstreamProducts priceRange={priceRange} products={filteredProducts} />
          )}
        </Box>
        {/* Shop Product End */}
      </Flex>
