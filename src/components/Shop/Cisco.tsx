...
export const Cisco: React.FC = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      if (selectedBrands.includes("cisco") || selectedBrands.length === 0) {
        const response = await axios.get(brandUrls["cisco"]);
  ...
  const handleBrandChange = (brand: string) => {
    if (brand === "cisco") {
      setSelectedBrands(["cisco"]);
    } else {
      setSelectedBrands((prevBrands) => {
        const updatedBrands = prevBrands.includes(brand)
          ? prevBrands.filter((b) => b !== brand)
          : [...prevBrands, brand].filter((b) => b !== "cisco");
        return updatedBrands.length === 0 ? ["cisco"] : updatedBrands;
      });
    }
  };
      ...
      <Flex flexWrap="wrap" gap={5}>
        ...
        <Box flex="1">
          {loading ? (
            ...
            <CiscoProducts priceRange={priceRange} products={filteredProducts} />
          )}
        </Box>
        {/* Shop Product End */}
      </Flex>
