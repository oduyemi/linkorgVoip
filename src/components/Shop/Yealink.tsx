...
import { YealinkProducts } from "./YealinkProducts";


export const Yealink: React.FC = () => {
  const [products, setProducts] = useState<Product[]>([]);
  const [filteredProducts, setFilteredProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [selectedBrands, setSelectedBrands] = useState<string[]>(["yealink"]);
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      if (selectedBrands.includes("yealink") || selectedBrands.length === 0) {
        const response = await axios.get(brandUrls["yealink"]);
  ...
  const handleBrandChange = (brand: string) => {
    if (brand === "yealink") {
      setSelectedBrands(["yealink"]);
    } else {
      setSelectedBrands((prevBrands) => {
        const updatedBrands = prevBrands.includes(brand)
          ? prevBrands.filter((b) => b !== brand)
          : [...prevBrands, brand].filter((b) => b !== "yealink");
        return updatedBrands.length === 0 ? ["yealink"] : updatedBrands;
      });
    }
  };
  ...
  return (
    <Box p={5}>
      <Flex flexWrap="wrap" gap={5}>
        ...
        {/* Shop Product Start */}
        <Box flex="1">
          <YealinkProducts priceRange={priceRange} products={filteredProducts} />
        </Box>
        {/* Shop Product End */}
      </Flex>
    </Box>
